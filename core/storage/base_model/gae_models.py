    ...
    def get(cls, entity_id, strict=True):
        ...
        if strict and entity is None:
            raise cls.EntityNotFoundError(
                'Entity for class {0!s} with id {1!s} not found'.format(cls.__name__, entity_id))
        return entity
    ...
    def get_new_id(cls, entity_name):
        ...
        for i in range(MAX_RETRIES):
            new_id = utils.convert_to_hash(
                '{0!s}{1!s}'.format(entity_name, utils.get_random_int(RAND_RANGE)),
                ID_LENGTH)
...
class VersionedModel(BaseModel):
    ...
    def _trusted_commit(
        ...
        if not isinstance(commit_cmds, list):
            raise Exception(
                'Expected commit_cmds to be a list of dicts, received {0!s}'.format(commit_cmds))
    ...
    def delete(self, committer_id, commit_message, force_deletion=False):
        if force_deletion:
            ...
            CMD_DELETE = '{0!s}_mark_deleted'.format(self._AUTOGENERATED_PREFIX)
            commit_cmds = [{
                'cmd': CMD_DELETE
            }]
    ...
    def commit(self, committer_id, commit_message, commit_cmds):
        ...
        for commit_cmd in commit_cmds:
            if 'cmd' not in commit_cmd:
                raise Exception(
                    'Invalid commit_cmd: {0!s}. Expected a \'cmd\' key.'.format(commit_cmd))
            if commit_cmd['cmd'].startswith(self._AUTOGENERATED_PREFIX):
                raise Exception(
                    'Invalid change list command: '.format(*commit_cmd['cmd']))
    ...
    def revert(cls, model, committer_id, commit_message, version_number):
        ...
        if not model.ALLOW_REVERT:
            raise Exception(
                'Reverting of objects of type {0!s} is not allowed.'.format(model.__class__.__name__))

        CMD_REVERT = '{0!s}_revert_version_number'.format(model._AUTOGENERATED_PREFIX)
        commit_cmds = [{
            'cmd': CMD_REVERT,
    ...
    def get_snapshots_metadata(
        ...
        for ind, model in enumerate(returned_models):
            if model is None:
                raise Exception(
                    'Invalid version number {0!s} for model {1!s} with id {2!s}'.format(version_numbers[ind], cls.__name__, model_instance_id))
