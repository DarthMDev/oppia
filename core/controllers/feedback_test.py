...
class FeedbackThreadPermissionsTests(test_utils.GenericTestBase):
    ...
    def setUp(self):
        ...
        response = self.testapp.get('/create/{0!s}'.format(self.EXP_ID))
        self.csrf_token = self.get_csrf_token_from_response(response)
        self.post_json('{0!s}/{1!s}'.format(
    ...
    def test_non_logged_in_users_can_view_threads_and_messages(self):
        ...
        response_dict = self.get_json(
            '{0!s}/{1!s}'.format(feconf.FEEDBACK_THREADLIST_URL_PREFIX, self.EXP_ID))
...
class FeedbackThreadIntegrationTests(test_utils.GenericTestBase):
    ...
    def test_no_username_shown_for_logged_out_learners(self):
        ...
        response = self.testapp.get('/create/{0!s}'.format(NEW_EXP_ID))
        csrf_token = self.get_csrf_token_from_response(response)
        self.post_json(
            '/explorehandler/give_feedback/{0!s}'.format(NEW_EXP_ID),
            {
                'state_name': None,
                'subject': 'Test thread',
        ...
        response_dict = self.get_json(
            '{0!s}/{1!s}'.format(feconf.FEEDBACK_THREADLIST_URL_PREFIX, NEW_EXP_ID))
        threadlist = response_dict['threads']
        self.assertIsNone(threadlist[0]['original_author_username'])

        response_dict = self.get_json('{0!s}/{1!s}/{2!s}'.format(
    ...
    def test_message_id_assignment_for_multiple_posts_to_same_thread(self):
        ...
        def _get_username(n):
            return 'editor{0!s}'.format(n)

        def _get_email(n):
            return '{0!s}@example.com'.format(n)
        ...
        for num in range(NUM_USERS):
            ...
            csrf_token = self.get_csrf_token_from_response(response)
            self.post_json(thread_url, {
                'text': 'New Message {0!s}'.format(num)
            }, csrf_token)
            self.logout()
